...
def algoritmo_genetico(num_estaciones, max_generaciones, poblacion_size):
    ...
    def crear_individuo():
        return [random.randint(1, 5) for _ in range(num_estaciones)]

    def evaluar_individuo(individuo):
        # Evaluación simplificada (costo basado en el número de operarios)
        costo = sum(individuo) * 10  # Costos asociados (puedes ajustarlo)
        return costo,
...
def simulacion_dinamica(estaciones, recursos):
    ...
    tiempos_ensamblaje = [random.randint(1, 5) for _ in range(estaciones)]  # Simulación simplificada
    resultados = []
...
num_estaciones = st.sidebar.slider('Número de estaciones de ensamblaje', 1, 10, 5)
num_operarios = st.sidebar.slider('Número de operarios por estación', 1, 10, 3)
max_generaciones = st.sidebar.slider('Número de generaciones del algoritmo genético', 10, 100, 50)
poblacion_size = st.sidebar.slider('Tamaño de la población', 10, 100, 50)
...
if st.sidebar.button("Ejecutar Optimización"):
    with st.spinner("Calculando..."):
        ...
        st.write(f"Costo total de la mejor asignación: {min(resultados_optim[0])}")
...
# Ejemplo de gráfico de barras para la asignación de operarios
asignacion_operarios = [random.randint(1, 10) for _ in range(num_estaciones)]  # Esto se genera aleatoriamente
