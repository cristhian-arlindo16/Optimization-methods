...
# 1. Algoritmo Genético Mejorado

def algoritmo_genetico(num_estaciones, max_generaciones, poblacion_size, costo_operarios, costo_maquinas):
    creator.create("FitnessMin", base.Fitness, weights=(-1.0,))  # Minimizar costo
    creator.create("Individual", list, fitness=creator.FitnessMin)

    def crear_individuo():
        return [random.randint(1, 10) for _ in range(num_estaciones)]  # Asignar operarios aleatorios

    def evaluar_individuo(individuo):
        costo_total = 0
        for i, operarios in enumerate(individuo):
            costo_total += operarios * costo_operarios  # Costo por operarios
            costo_total += 3 * costo_maquinas  # Costo fijo por máquinas por estación
        return costo_total,  # Tupla de un solo valor
...
# Costos
costo_operarios = st.sidebar.slider('Costo por operario ($)', 10, 100, 50)
costo_maquinas = st.sidebar.slider('Costo por máquina ($)', 100, 1000, 500)

# Botón para ejecutar la optimización
if st.sidebar.button("Ejecutar Optimización"):
    with st.spinner("Calculando..."):
        # Ejecutar optimización
        resultados_optim = algoritmo_genetico(num_estaciones, max_generaciones, poblacion_size, costo_operarios, costo_maquinas)
        ...
        st.write(f"Costo total de la mejor asignación: ${costo_mejor:.2f}")
